...
Exemplos de sistemas de banco de dados: MySQL, Postgres, Oracle.
...
Linguagem SQL, serial query language.
...
Quantidade de partos por tipo e data
```{r}
tab_sinasc |>
  mutate(DTNASC = as_date(DTNASC)) |>
  group_by(DTNASC, PARTO) |>
  summarise(freq = n()) |>
  ungroup() |>
  collect() |>
  ggplot(aes(x = DTNASC, y = freq, col = PARTO)) +
  geom_line()
```
